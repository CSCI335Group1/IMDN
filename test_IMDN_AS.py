...
def parse_img_name(imname, extension):
    result = imname.split('/')[-1]
    if not opt.use_as:
        result = result.split('.')[0] + 'x' + str(opt.upscale_factor) + extension
    return result
...
model = architecture.IMDN_AS() if opt.use_as else architecture.IMDN(upscale=opt.upscale_factor) 
...
for imname in filelist:
    if opt.use_as:
        ...
        im_l = cv2.imread(opt.test_lr_folder + parse_img_name(imname, ext))[:, :, [2, 1, 0]]
    else:
        im_gt = cv2.imread(imname, cv2.IMREAD_COLOR)[:, :, [2, 1, 0]]  # BGR to RGB
        im_gt = utils.modcrop(im_gt, opt.upscale_factor)
        im_l = cv2.imread(opt.test_lr_folder + parse_img_name(imname, ext), cv2.IMREAD_COLOR)[:, :, [2, 1, 0]]  # BGR to RGB
    ...
    with torch.no_grad():
        ...
        if (h % 4 == 0 and w % 4 == 0) or not opt.use_as:
            start.record()
            out = model(im_input)
            end.record()
    ...
    if opt.use_as:
        ...
        sr_img = utils.shave(out_img, crop_size)
        im_gt = utils.shave(im_gt, crop_size)
